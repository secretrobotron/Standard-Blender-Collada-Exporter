...
class DaeLibrary(DaeElement):
	...
	def FindObject(self,url):
		for i in self.items:
			#Debug.Debug(str(i.id) + "?" + str(url), 'FEEDBACK')
			if i.id == url:
				return i
		return None
...
class DaeLight(DaeElement):
	...
	class DaePoint(DaeTechniqueCommon):
		...
		def SaveToXml(self, daeDocument):
			...
			AppendTextChild(node.firstChild,DaeSyntax.INTENSITY, self.intensity)
			AppendTextChild(node.firstChild,DaeSyntax.DISTANCE, self.distance)

			Debug.Debug(str(self.intensity),'FEEDBACK')
			return node
