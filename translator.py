...
#old								curve.addBezier((time,value))
	...
	def SaveToDae(self, ipo, targetDaeNode, joint=None, bPose=None, bParentMatrix=None, bArmatureObject=None):
			...
			for curve in curves:
				...
				if cName.startswith("Loc") or cName.startswith("Rot") or cName.startswith("Scale"):
					...
					for frameNumber in frames:
						anit = ani.setdefault(float(frameNumber),{'X':None, 
																											'Y':None, 
																											'Z':None,
																											'bezier': curve.bezierPoints[i],
																											'interpolation':interpolation, 
																											'intangent': curve.bezierPoints[i].vec[0],
																											'outtangent': curve.bezierPoints[i].vec[2]})
						i+=1
		...
		for name, animation in animations.iteritems():
			...
			if name.startswith(collada.DaeSyntax.TRANSLATE) or name == collada.DaeSyntax.SCALE:
				vals = self.CreateSourceOutput(daeAnimation, len(animation),  "xyz", name[-1])
			...
			daeSourceInTangent = vals[4]
			inTangentArray = vals[5]
	...
	def CreateSourceOutput(self, daeAnimation, count, type, label=None):
		...
		if type == "xyz":
			accessorOutput.AddParam(label, 'float')
			accessorInterpolation.AddParam(label, 'Name')
		...
		accessorInTangent = collada.DaeAccessor()
		daeSourceInTangent.techniqueCommon.accessor = accessorInTangent
		accessorInTangent.source = inTangentArray.id
		accessorInTangent.count = count
		accessorInTangent.AddParam('X','float')
		accessorInTangent.AddParam('Y','float')
		...
		accessorOutTangent = collada.DaeAccessor()
		daeSourceOutTangent.techniqueCommon.accessor = accessorOutTangent
		accessorOutTangent.source = outTangentArray.id
		accessorOutTangent.count = count
		accessorOutTangent.AddParam('X','float')
		accessorOutTangent.AddParam('Y','float')

		return [daeSourceOutput, outputArray, daeSourceInterpolation, interpolationArray, daeSourceInTangent, inTangentArray, daeSourceOutTangent, outTangentArray]
